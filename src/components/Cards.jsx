...
const Card = ({ text, checkbox, id }) => {
  ...
  const updateValue = (checkbox, oldText = text, id) => {};
  const imageDeleteOneTask = (id) => {
    axios.delete(`${url}/deleteTask?id=${id}`).then((result) => {
  ...
  return (
    <div id={id} className="task">
      ...
      <p
        id={`text-${id}`}
        className={!checkbox ? "text-task" : "text-task done-text"}>
        {text}
      </p>
      ...
      <i title="Delete" onClick={() => imageDeleteOneTask(id)}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512">
